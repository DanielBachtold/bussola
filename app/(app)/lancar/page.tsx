...
export default async function LancarPage() {
  const [accounts, categories, rules, recent] = await Promise.all([
    listAccounts(), listCategories(), listRules(pool), listTransactions({ month: currentMonth(), status: 'pending', limit: 20 }),
  ]);
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
      ...
      <QuickAdd accounts={accounts} categories={categories} rules={rules} />
      <details className="card p-4">
        <summary className="cursor-pointer font-medium text-[14px]">Formulário completo</summary>
        <div className="mt-3"><FullForm accounts={accounts} categories={categories} /></div>
      </details>
      <section className="card p-4 flex flex-col gap-2">
        <h2 className="font-semibold">Lançados neste mês, aguardando extrato</h2>
        <TxList items={recent} categories={categories} compact emptyText="Nada pendente. Quando subir o extrato, o que você lançou aqui é conciliado automaticamente." />
      </section>
    </div>
  );
}
