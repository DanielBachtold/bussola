...
export function PayForm(props: { cardId: number; month: string; total: number; checking: Account[]; defaultDate: string }) {
  const [open, setOpen] = useState(false);
  if (!open) return <button className="btn btn-primary btn-sm" onClick={() => setOpen(true)}>Registrar pagamento</button>;
  return <PayFormBody {...props} close={() => setOpen(false)} />;
}

function PayFormBody({ cardId, month, total, checking, defaultDate, close }: { cardId: number; month: string; total: number; checking: Account[]; defaultDate: string; close: () => void }) {
