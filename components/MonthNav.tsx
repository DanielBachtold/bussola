...
export function MonthNav({ month, basePath, extra = '' }: { month: string; basePath: string; extra?: string }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <Link href={`${basePath}?m=${prev}${extra}`} className="btn btn-ghost !p-0 w-11 h-11 md:w-9 md:h-9" aria-label="Mês anterior"><ChevronLeft size={18} /></Link>
      <select
      ...
      <Link href={`${basePath}?m=${next}${extra}`} className="btn btn-ghost !p-0 w-11 h-11 md:w-9 md:h-9" aria-label="Próximo mês"><ChevronRight size={18} /></Link>
